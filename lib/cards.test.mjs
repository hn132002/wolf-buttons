...
assert.equal(byKey.has("隱藏"), true);
...
assert.equal(storedByKey.get("A").cardCount, 3);
...
assert.deepEqual(
  projectPublicCardCategories(
    [storedCategory("visible", 0), storedCategory("empty public", 1)],
    [
      projectionCard("visible-card", ["visible"], 0),
      projectionCard("orphan-card", ["orphan public"], 1),
      projectionCard("uncategorized-card", [], 2),
    ]
  ).map((category) => category.key),
  ["visible", "orphan public", UNCATEGORIZED_CATEGORY_KEY]
);
assert.deepEqual(
  projectStoredCardCategories(
    [
...
assert.deepEqual(
  resolveCardCategoryKeys(["互動", "💬 互動", "💬 溝通互動"], [
    { key: "💬 溝通互動", name: "互動", emoji: "💬" },
  ]),
  { ok: true, categories: ["💬 溝通互動"] }
);
assert.deepEqual(
  resolveCardCategoryKeys(["不存在"], [
    { key: "💬 溝通互動", name: "互動", emoji: "💬" },
  ]),
  { ok: false, error: "找不到分類：不存在", status: 400 }
);
assert.deepEqual(
  resolveCardCategoryKeys(
    ["互動", "新分類"],
...
const cardsRouteSource = readFileSync(
  new URL("../app/api/cards/route.ts", import.meta.url),
  "utf8"
);
const cardRouteSource = readFileSync(
  new URL("../app/api/cards/[id]/route.ts", import.meta.url),
  "utf8"
);
...
const adminCategoryCardsRouteSource = readFileSync(
  new URL("../app/api/admin/card-categories/[id]/cards/route.ts", import.meta.url),
  "utf8"
);
...
assert.equal(adminClientSource.includes("<datalist"), true);
assert.equal(adminClientSource.includes("刪除分類"), true);
assert.equal(adminClientSource.includes("刪除此分類字卡"), true);
assert.equal(adminClientSource.includes("仍有 {category.cardCount} 張字卡，無法刪除"), false);
assert.equal(formatCardCategoryName({ name: "行程時間", emoji: "📅" }), "📅 行程時間");
assert.equal(cardsRouteSource.includes("allowMissing: true"), true);
assert.equal(cardRouteSource.includes("allowMissing: true"), true);
assert.equal(adminClientSource.includes("新增分類 ${result.createdCategories ?? 0} 個"), true);
...
assert.equal(adminCategoryCardsRouteSource.includes("deleteMany"), true);
assert.equal(adminCategoryCardsRouteSource.includes("categories: { has: category.key }"), true);
...
const replacePreview = parseBatchTsv(
  [
    header,
    "card-1\t⭐常用|⭐常用\t謝謝\tありがとう\t謝謝！\tありがとうございます！\tThank you!",
    "\t⭐常用\t等一下\t待って\t請等一下。\t少し待ってください。\tPlease wait.",
  ].join("\n"),
  cards,
  "replace"
);
...
assert.equal(replacePreview.hides.length, 0);
assert.equal(replacePreview.replaceDeletes.length, 1);
assert.deepEqual(replacePreview.rows[0].categories, ["⭐常用"]);

const upsertPreview = parseBatchTsv(
  [
    header,
    "card-1\t⭐常用\t謝謝\tありがとう\t謝謝！\tありがとうございます！\tThank you!",
  ].join("\n"),
  cards,
  "upsert"
);
...
const uncategorizedPreview = parseBatchTsv(
  [
    header,
    "\t未分類\t\t\t\t\t",
  ].join("\n"),
...
assert.deepEqual(uncategorizedPreview.errors, []);
assert.deepEqual(uncategorizedPreview.rows[0].categories, []);

const errorPreview = parseBatchTsv(
  [
    header,
    "missing-id\t⭐常用\t謝謝\tありがとう\t謝謝！\tありがとうございます！\t",
  ].join("\n"),
  cards,
  "upsert"
);
...
const serverPreview = previewBatchRows(
  [{ ...upsertPreview.rows[0], categories: ["未分類"] }],
  cards,
  "upsert"
);
assert.deepEqual(serverPreview.errors, []);
assert.deepEqual(serverPreview.rows[0].categories, []);

const resolvedBatchPreview = previewBatchRows(
  [{ ...upsertPreview.rows[0], categories: ["常用", "⭐常用"] }],
  cards,
  "upsert",
  [{ key: "⭐常用", name: "常用", emoji: "⭐" }]
);
assert.deepEqual(resolvedBatchPreview.errors, []);
assert.deepEqual(resolvedBatchPreview.rows[0].categories, ["⭐常用"]);

const newBatchCategoryPreview = previewBatchRows(
  [{ ...upsertPreview.rows[0], categories: ["不存在"] }],
  cards,
  "upsert",
  [{ key: "⭐常用", name: "常用", emoji: "⭐" }]
);
assert.deepEqual(newBatchCategoryPreview.errors, []);
assert.deepEqual(newBatchCategoryPreview.rows[0].categories, ["不存在"]);

const mixedBatchCategoryPreview = previewBatchRows(
  [{ ...upsertPreview.rows[0], categories: ["常用", "新分類"] }],
  cards,
  "upsert",
  [{ key: "⭐常用", name: "常用", emoji: "⭐" }]
);
assert.deepEqual(mixedBatchCategoryPreview.errors, []);
assert.deepEqual(mixedBatchCategoryPreview.rows[0].categories, ["⭐常用", "新分類"]);
